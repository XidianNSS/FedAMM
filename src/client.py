...
class Client:
    ...
    def train(self):
        ...
        accumulated_gradients = {name: torch.zeros_like(param) for name, param in self.local_model.named_parameters() if
                                 'lora_' not in name}

        initial_non_lora_params = {name: param.clone() for name, param in self.local_model.named_parameters() if
                                   'lora_' not in name}
        ...
        non_lora_param_changes = {name: param - initial_non_lora_params[name] for name, param in
                                  self.local_model.named_parameters() if 'lora_' not in name}
        self.local_model.to('cpu')
        return sum(epoch_loss) / len(epoch_loss), end_time - start_time, non_lora_param_changes
    ...
    def inference_globaltestset(self, mode='all', globaltestloader=None):
        self.local_model.to(self.device)
        self.local_model.eval()
        loss, total, correct = 0.0, 0.0, 0.0
        ...
        count = 1
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(globaltestloader):
                images, labels = images.to(self.device), labels.to(self.device)
