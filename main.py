...
def exp_parameter(args, logger):
    ...
    logger.info(f'Communication Rounds: {args.epochs}')
    logger.info(f'Client Number: {args.num_users}')
    logger.info(f'Client Frac: {args.frac}')
    ...
    logger.info(f'noniid: {args.noniid}')
...
def train(args, logger=None):
    ...
    for t in range(args.repeat):  # repeat args.repeat times
        ...
        logFileName = f'./log/FedAMM_dataset{args.dataset}_model{args.model}_frac{args.frac}_policy{args.policy}_round{args.epochs}_epoch{args.local_ep}_pep{args.local_p_ep}_ConvR{args.Conv_r}_LinearR{args.Linear_r}_optimizer{args.optimizer}_lr{args.lr}_iid{args.iid}_{args.noniid}_{args.alpha}_trainnum{args.train_num}_seed{args.seed}_lambda{args.momentum_coefficient}.log'
